...
const apiUrl = "https://test.biri.link/api/daily-number4"; // Use the relative URL since the server is running on the same domain
...
fetchDailyNumber().then(() => {
  dailyWord = dict[dailyNumber];
  gunlukOn.innerHTML = "Quotidien #"+ dailyCounter;
...
const chooseDailyWord = () => {
  // Check if the user has already played the daily word challenge today
  const lastPlayedDate = localStorage.getItem("PNTlastPlayedDateFR");
  //console.log(lastPlayedDate);

  //console.log(lastPlayedDate === currentDate);
  if (lastPlayedDate === currentDate) {
    showMessage("NOUVEAU PUZZLE DEMAIN!");
...
const completeDailyWord = () => {
  ...
  if (!ipucuMu) {
    ...
    localStorage.setItem("PNTlastPlayedDateFR", currentDate);
    localStorage.setItem("PNTisDailyMode", JSON.stringify(isDailyMode));
    if (!ipucuMu) {
    ...
    setTimeout(() => {
      ...
      link.textContent = "C'est quoi " + wordle + "?";
      link.href = `https://www.google.com/search?q=C%27est+quoi+${encodeURIComponent(
        wordle
      )}`;
...
lastPlayedDate = localStorage.getItem("PNTlastPlayedDateFR");
...
const checkRow = () => {
  ...
  if (currentTile > 4) {
    if (!checkwords.includes(guess)) {
      showMessage("MOT PAS DANS LA LISTE !");
      return;
    } else {
      // Word found
...
const infoPopup = () => {
  ...
  "COMMENT JOUER?" +
   "\n" +
   "\nTrouvez PENTALETTRE en 6 tentatives." +
   "\n" +
   "\nChaque supposition doit être un mot de 5 lettres. Appuyez sur Entrée pour soumettre." +
   "\n" +
   "\nAprès chaque supposition, les couleurs des cases changeront en fonction de votre supposition." +
   "\n" +
   "\n\u{1F7E9} les lettres sont dans le mot et au bon endroit." +
   "\n\u{1F7E8} lettres sont dans le mot mais au mauvais endroit." +
   "\n\u{2B1B} la lettre n'existe pas dans le mot.";
...
const showUserStatistics = () => {
  ...
  const statisticsString = `TOTAL DES MOTS TROUVÉS ${userStatistics.totalWordsFound}\n`;
  ...
  const attemptsTextList = Object.entries(attemptsCountMap).map(
    ([attempts, count]) => {
      return `Trouvé ${count} mot(s) en ${attempts}. devinez.\n`;
    }
  );
